...
pub async fn delete_serve(_id: web::Path<i32>) -> impl Responder {
    ...
    let _category = serve_categories.filter(schema::serve_categories::id.eq(&_serve.id)).load::<ServeCategories>(&_connection).expect("E");
    diesel::update(_category)
            .set(schema::serve_categories::serve_count.eq(_category.serve_count - 1))
            .get_result::<ServeCategories>(&_connection)
            .expect("Error.");
