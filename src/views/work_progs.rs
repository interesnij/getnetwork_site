...
pub async fn get_work_page(session: Session, req: HttpRequest, param: web::Path<(String,String)>) -> actix_web::Result<HttpResponse> {
    ...
    if is_ajax == 0 {
        ...
        let _cats = Categories::get_categories_for_types(5);
...
pub async fn work_category_page(session: Session, req: HttpRequest, _id: web::Path<String>) -> actix_web::Result<HttpResponse> {
    ...
    if is_ajax == 0 {
        ...
        if is_signed_in(&session) {
            let _request_user = get_request_user_data(&session);
            let (object_list, next_page_number) = Categories::get_works_list(_category.id, page, 20, _request_user.is_superuser());
            if is_desctop {
                #[derive(TemplateOnce)]
            ...
            let (object_list, next_page_number) = Categories::get_works_list(_category.id, page, 20, false);
